...
int	ft_init_mutex(t_rules *rules)
{
	...
	while (i < rules->nb_philo)
	{
		if (pthread_mutex_init(&rules->forks[i], NULL) != 0)
			return (ft_dst_forks(rules, i), ft_dst_last_meal(rules, i),
				ft_free_alloc(rules), ft_destroy_nb_meal(rules, i), 1);
		if (pthread_mutex_init(&rules->philos[i].mutex_last_meal, NULL) != 0)
			return (ft_dst_forks(rules, i + 1), ft_dst_last_meal(rules, i),
				ft_destroy_nb_meal(rules, i), ft_free_alloc(rules), 1);
	...
	if (pthread_mutex_init(&rules->mutex_print, NULL) != 0)
		return (ft_dst_forks(rules, rules->nb_philo),
			ft_dst_last_meal(rules, rules->nb_philo), ft_free_alloc(rules), 1);
	if (pthread_mutex_init(&rules->mutex_stop, NULL) != 0)
		return (ft_dst_forks(rules, rules->nb_philo),
			ft_dst_last_meal(rules, rules->nb_philo),
			pthread_mutex_destroy(&rules->mutex_print),
			ft_free_alloc(rules), 1);
	return (0);
}
